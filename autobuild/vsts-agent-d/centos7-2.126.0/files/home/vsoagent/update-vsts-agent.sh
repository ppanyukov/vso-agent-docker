...
then
    VSTS_LATEST_URL="https://github.com/Microsoft/vsts-agent/releases/download/v${VSTS_LATEST_VERSION}/vsts-agent-rhel.7.2-x64-${VSTS_LATEST_VERSION}.tar.gz"
    VSTS_LATEST_URL2="https://vstsagentpackage.azureedge.net/agent/${VSTS_LATEST_VERSION}/vsts-agent-linux-x64-${VSTS_LATEST_VERSION}.tar.gz"

    echo "Will download from ${VSTS_LATEST_URL} or ${VSTS_LATEST_URL2}"
    (
        wget \
            ${VSTS_LATEST_URL} -O "${DOWNLOAD_DIR}/${VSTS_TAR_NAME}" \
        || \
        wget \
            ${VSTS_LATEST_URL2} -O "${DOWNLOAD_DIR}/${VSTS_TAR_NAME}" \
    )
